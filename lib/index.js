...
exports.SuperProvider = void 0;
const providers_1 = require("@ethersproject/providers");
// https://advancedweb.hu/how-to-add-timeout-to-a-promise-in-javascript/
const utils_1 = require("./utils");
class SuperProvider extends providers_1.BaseProvider {
    constructor(providers, chainId, options) {
        ...
        if (providers.length < 3) {
            console.warn("SuperProvider needs at least 3 sub-providers to be effective. The more the better.");
        }
    ...
    async perform(method, params) {
        const promiseGen = async (p) => {
            try {
                return await (0, utils_1.promiseTimeout)(p.perform(method, params), this.stallTimeout);
            }
            catch (e) {
                this.banProvider(p);
        ...
        const recursiveRetry = async () => {
            ...
            try {
                ...
                await (0, utils_1.expRetryTimeout)(tries);
                recursiveRetry();
            }
        };
    ...
    banProvider(provider) {
        if (this.providersPool.length <= 1)
            return;
    ...
    async benchmarkProviders() {
        ...
        const run = async () => {
            ...
            const promises = (0, utils_1.shuffleArray)(this.providers).map(async (provider) => {
                const startTime = Date.now();
                try {
                    const call = provider.perform("getBlockNumber", {});
                    const blockHex = await (0, utils_1.promiseTimeout)(call, this.stallTimeout);
            ...
            const currentBlock = (0, utils_1.quantile)(results.map((p) => p.block), 0.75);
        ...
        const median = (0, utils_1.quantile)(sortedResults.map((r) => r.avgResponseTime), 0.5);
        const filteredResults = sortedResults.filter((r) => r.avgResponseTime <= median * 1.5);
        console.log(`SuperProvider: benchmark x${this.benchmarkRuns} finished. sorted providers:`, filteredResults.map((p) => `${p.provider.connection?.url} | avg ${parseInt(p.avgResponseTime)}ms`));
...
exports.SuperProvider = SuperProvider;
